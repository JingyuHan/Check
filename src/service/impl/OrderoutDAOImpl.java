...
public class OrderoutDAOImpl implements OrderoutDAO{
	...
	public List<Orders> queryOrderoutLike(String b_date,String e_date,String user,String customer,String note,String gname,int beginIndex,int num){
		...
		try{
			Session session = MyHibernateSessionFactory.getSessionFactory().getCurrentSession();
			tx = session.beginTransaction();
			hql = "from Orders where type = false and user like '%"+user+"%' and customer like '%"+customer+"%'";
			if(!"".equals(b_date)){
				if(!"".equals(e_date)){
			...
			hql += " and note like '%"+note+"%' and oid in(SELECT ooid from Order_out WHERE gname LIKE '%"+gname+"%') order by oid";
			Query query = session.createQuery(hql);
			query.setFirstResult(beginIndex);
			query.setMaxResults(num);
			list = query.list();
